...
- "Pontos de Atenção" e "Oportunidades" têm no máximo 3 bullets CADA, "Pendências" no máximo 3 — se houver mais do que isso, priorize o mais recente/relevante e descarte o resto (mesmo que fosse um ponto válido isolado). Em "Pendências" especificamente, um ERRO ou INCONSISTÊNCIA concreta encontrada (ex.: parâmetro de estoque/liquidez errado, dado cadastral incorreto) pesa mais que uma tarefa administrativa de rotina (ex.: enviar link de acesso, mandar relatório) — priorize o achado técnico quando os dois competirem pelo mesmo slot. Reuniões diferentes com o MESMO padrão viram um bullet só, citando as datas juntas — nunca um bullet por reunião. Cada bullet é UMA frase curta, não um parágrafo com várias orações separadas por ";" — corte pro fato que mais pesa, não tente caber tudo.`;
}
