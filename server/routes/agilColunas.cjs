...
router.get('/', (req, res) => {
  res.json(repo.get('AgilColunas'));
});

router.post('/', validar(agilColunaCreateSchema), (req, res) => {
  res.json(colunasDominio.criar(repo, req.body));
});
...
router.put('/reorder', validarLote(agilReorderColunaItemSchema), (req, res) => {
  res.json(colunasDominio.reordenar(repo, req.body));
});

router.put('/:id', validar(agilColunaUpdateSchema), (req, res) => {
  const updated = colunasDominio.atualizar(repo, req.params.id, req.body);
  if (!updated) return res.status(404).json({ error: 'Coluna não encontrada.' });
  res.json(updated);
});

router.delete('/:id', (req, res) => {
  const found = colunasDominio.remover(repo, req.params.id);
