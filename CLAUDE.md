...
- **Peso da release** (`DEPS_SERVIDOR` em `server/scripts/publicarRelease.cjs`): o staging instala SÓ os 10 pacotes que o servidor exige em runtime, do zero — não copia o `node_modules` inteiro. Até a v1.2.34 ele levava os 194 pacotes de `dependencies`, incluindo os de FRONTEND (jspdf 28,8 MB, lucide-react 28,8 MB, cache `.vite` 16,5 MB) que já estão compilados dentro de `dist/`: 85 MB de download e 274 MB / 20.820 arquivos pra descompactar numa pasta sincronizada por OneDrive — era essa a lentidão das atualizações. Hoje: **58 MB / 160 MB / 8.031 arquivos**. `npm prune --omit=dev` não resolvia porque os pacotes de frontend estão em `dependencies` mesmo (o `npm run build` precisa deles). `verificarDepsDoServidor()` roda a cada publish e FALHA se algum `require` do servidor estiver fora da lista — senão a release quebraria só na máquina de destino, com "Cannot find module".
- **Novidades por versão**: `NOVIDADES.md` na raiz, uma seção `## <versão>` com bullets escritos pro USUÁRIO (não mensagem de commit). `publicarRelease` copia o arquivo pra release E embute as novidades da versão no `latest.json` — o manifesto é o que permite a máquina que ainda NÃO atualizou mostrar o que vem na versão nova. A tela (Configurações → Sistema) mostra as da versão disponível quando há atualização, e as da instalada depois de atualizar.
...
**Atualização — o agente agora TAMBÉM redige e exporta ata (deixou de ser escopo separado)**: `server/ia/tools.cjs` ganhou `redigir_ata_reuniao` (chama `gerarAtaIA`, mesmo motor do botão "Gerar ata com IA", com `instrucaoPersonalizada` livre — "foca no financeiro" etc.) e `gerar_ata_pdf` (gera o PDF e devolve link `/uploads/...`). As duas são write-tools CONDICIONAIS: só gravam no evento/anexam quando `salvar`/`anexar` vem `true` — a descrição de cada ferramenta instrui o modelo a só mandar isso depois do usuário CONFIRMAR, nunca de cara (mesmo padrão de `corrigir_dossie_cliente`). `server/ia/ataTexto.cjs` e `server/ia/ataPdf.cjs` são ports server-side (CommonJS) de `src/utils/ata.ts`/`ataPdf.ts` — a duplicação é aceita de propósito (não dá pra importar TS/ESM de um `.cjs` de servidor sem build step) mas é risco real de divergência: qualquer mudança de formato/layout num lado precisa ser espelhada no outro.

**Bug real encontrado ao integrar isso**: o despachante de tool-calling (`orquestrador.cjs`, caminho Ollama, e `iaProvedor.cjs` `/interno/ferramenta`, caminho Claude CLI/MCP) chamava `ferramenta.executar(...)` **sem `await`**. Pra ferramenta síncrona nunca importou; pra ferramenta ASYNC (`reanalisar_cliente` já era async antes disso, e agora `redigir_ata_reuniao` também) o resultado real virava uma `Promise` gravada como `{}` no histórico (`JSON.stringify` numa Promise não serializa nada), e uma REJEIÇÃO virava unhandled rejection — mesma classe do crash de `consultarLimiteConta` (02/09/2026). Corrigido com `await` nos dois despachantes; `await` num valor não-Promise (ferramenta sync) simplesmente devolve o próprio valor, então não quebrou nada síncrono.

### Conceito da carteira explicado com dado real, não de memória do modelo

Testando o agente ao vivo com 10 perguntas reais, achei um bug de fato: "quantos clientes ativos" contava só pelo campo bruto `estado` (Ativo/Inativo), ignorando `status` (Suspenso/Atendido pelo Marco/Problemas Externos não contam como atendimento) e pausa temporária — um cliente "Atendido pelo Marco" contava como ativo, inflando o número. `buscar_clientes` agora expõe um campo `ativo` já calculado com `isClienteAtivo` (a mesma regra do resto do app), e a descrição da ferramenta instrui o modelo a usar ESSE campo, nunca `estado` sozinho, pra responder "está ativo?"/"quantos ativos?".

Pra generalizar (usuário pediu explicitamente um "especialista em conceitos", mesmo eu tendo sugerido a alternativa mais simples de só ensinar via system prompt): `explicar_conceito_carteira` (`server/ia/tools.cjs`) — ferramenta nova que explica regra de negócio da Carteira (cliente ativo, cadência, Cobertura x Saúde da Carteira, risco, ação sem sucesso, pausa temporária, segmento x linha — glossário em `server/ia/conceitosCarteira.cjs`) **sempre com um recorte de dado real** da carteira do usuário, nunca só teoria. Faz uma chamada de modelo **sem ferramentas** (`clienteLLM().gerarJSON`, mesmo padrão já usado em `analiseCliente.cjs`), não o loop completo `conversar()` — reusar o loop dentro de uma ferramenta que está na mesma lista `FERRAMENTAS` arriscaria a ferramenta se chamar recursivamente. Medida como `origem: 'conceito'` em `UsoIA`, mesma preocupação de "chamada de modelo que não passa pelo chat não aparecia no painel de consumo" já resolvida pra análise automática.
